...
def perform_step(atoms, l, frame, SCAN_DIR, H_idx, C_idx, O_idx, k, rc_step, opt_thereshold=0.05, opt_steps=10000, frozen_idx=[]):
    ...
    traj_k_path = os.path.join(SCAN_DIR, f'scan_{frame}_{l}.traj')
    log_k_path = os.path.join(SCAN_DIR, f'scan_{frame}_{l}.log')

    opt = LBFGS(atoms, trajectory=traj_k_path, logfile=log_k_path)
...
def compute_rc(atoms, H_idx, C_idx, O_idx):
    ...
    positions = atoms.positions
    pos_H = positions[H_idx-1]
    pos_C = positions[C_idx-1]
    pos_O = positions[O_idx-1]
...
pdb_path = os.path.join(SRC_PATH, f'opt_{frame}.pdb')                    # .pdb from where the prmtop and inpcrd where generated (frame src)
...
# C10 abstraction
H_idx = 10583         # index of H atom
C_idx = 10581         # index of start bond atom X-H     Y
...
# scan options
rc_step = 0.2               # Å
rc_thereshold = 0.0         # stop scan when rc thereshold is reached
visualize = False
...
with open(csv_out, 'w') as f:
    ...
    while _rc < rc_thereshold:
        ...
        # save to file
        new_structure = os.path.join(SCAN_PATH, f'scan_{frame}_{l}.xyz')
        write(new_structure, atoms)
        
        if visualize:
            view(Trajectory(os.path.join(SCAN_PATH, f'scan_{frame}_{l}.traj')), viewer='ase')
