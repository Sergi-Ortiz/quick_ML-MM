...
def perform_step(atoms, l, frame, SCAN_DIR, H_idx, C_idx, O_idx, k, rc_step, opt_thereshold=0.05, opt_steps=10000, frozen_idx=[]):
    ...
    # combine constraints (previously deleted)
...
# general config for the constraint (testing) PRE-TAULI
# k = 291.54
# rc_step = 0.01
# opt_thereshold = 0.08

# for TS (slow but works) (post320)
# k = 3 * 291.54
# rc_step = 0.0002
# opt_thereshold = 0.08

# for products (post TS)
# k = 3 * 291.54
# rc_step = 0.0025
# opt_thereshold = 0.1

# GPU INITIAL
# initial frame optimization should be done at 0.001 to ensure a safe energy starting point
# GPU slow but steady, stable. Increase in energy ok! 
#k = 291.54                  # eV/Å^2
#rc_step = 0.001              # Å
#opt_thereshold = 0.01        # eV / Å
#save_it = 1                 # how often to save pdb and energy (in opt iterations). Debug: 1. Default: 10

# NOPE EXPLODES!!
# speed up with a larger rc step see convergence. Larger rc, lower opt thereshold to ensure energy stability
#k = 291.54                  # eV/Å^2
#rc_step = 0.005              # Å
#opt_thereshold = 0.008        # eV / Å
#save_it = 1                 # how often to save pdb and energy (in opt iterations). Debug: 1. Default: 10


# FUNCIONA PER >-0.8
# NEW ISSUE, FROM A CERTAIN ENERGY, k MUST BE LARGE ENOUGH TO PULL THE H, OTHERWISE THE RC GOES DOWN AGAIN LMAO
# also further stability is needed, so opt has been decreased slightly
# decreasing thereshold oes not solve anything
# new test: approaching TS, needs rc very very small. High cost. 
# si ho deixes que optimtzi molt acaba explotant. Millor fer pasets a lo bruto i no pensar massa en la opt. 
# GPU slow but steady, stable. Increase in energy ok! 
# comentari: el valor de k hauria de ser més gran si el step rc es més petit.
#k = 1 * 291.54                  # eV/Å^2
#rc_step = 0.0025              # Å
#opt_thereshold = 0.02        # eV / Å
#save_it = 5                 # how often to save pdb and energy (in opt iterations). Debug: 1. Default: 10

#---------------------------------------------------------------

# sometimes the maximum force increases randomly and impacts the total energy subtantially: POssible fix: microiterations. Lower rc_step. 
# test_1
# -1.33 a -1.13 en 22 min. 
#k = 1 * 291.54                  # eV/Å^2
#rc_step = 0.001              # Å
#opt_thereshold = 0.01        # eV / Å
#save_it = 40                 # how often to save pdb and energy (in opt iterations). Debug: 1. Default: 10

# afecta el step en pasos random de molt elevada energia? Resposta; 
# test_2
#k = 1.1 * 291.54                  # eV/Å^2
#rc_step = 0.0005              # Å
#opt_thereshold = 0.01        # eV / Å
...
#save_it = 20                 # how often to save pdb and energy (in opt iterations). Debug: 1. Default: 10

# post TS, 0.001 explodes eventuall
k = 6 * 291.54                  # eV/Å^2
rc_step = 0.0005              # Å
opt_thereshold = 0.02        # eV / Å
save_it = 10                 # how often to save pdb and energy (in opt iterations). Debug: 1. Default: 10
...
# fer que la k sigui tal que la força maxima sigui sempre menor a 10. 
# k en funció de rc. 



# scan options
rc_thereshold = 2.0         # stop scan when rc thereshold is reached
visualize = False
rst_frame = 15935            # frame from which to restart the scan (must be in the src dir). Default: None
energy_ref = -132201.749690           # value of the energy (in eV) of rst_frame. Default: None
#300, 900, 2620

# no entenc res, si k es massa petita, potser la rc disminueix en lloc d'augmentar.... weid...
# si k es massa gran, dona problemes d'estabilitat, pes no smooth. 
...
# initial structure (allows restart from any given scan frame)
if rst_frame is not None:
    pdb_path = os.path.join(SRC_PATH, f'scan_{frame}_{rst_frame}.pdb')               # restart scan from a given pdb
else:
...
mlmm_kwargs = dict(
    ...
    mm_device    = 'cuda',
    mm_cuda_idx  = 0,
    #mm_threads   = mm_threads,      # max number of threads available (only for CPU)
)
...
# read the initial (optimized)
atoms = read(opt_pdb)
atoms.calc = CALC_ASE

with open(csv_out, 'w') as f:
    f.write('j,rc [Å],energy [eV],DeltaE [kcal/mol],r_CH,r_OH\n')
...
while _rc < rc_thereshold:
    ...
    # get energy reference (first frame: opt frame)
    if energy_ref is None and l == 1:
        energy_ref = atoms.get_potential_energy(apply_constraint=False)
    ...
    if l % save_it == 0:
        ...
        # compute energy "as is"
        energy = atoms.get_potential_energy(apply_constraint=False) 
        
        # save structure file
        write(os.path.join(SCAN_PATH, f'scan_{frame}_{int(l+rst_frame)}.pdb'), atoms)

        # save results to a .csv (same as in QM/MM)
        with open(csv_out, 'a') as f:
            f.write(f'{int(l+rst_frame)},{_rc:.6f},{energy:.6f},{(energy-energy_ref)*EV2KCALMOL:.6f},{r_CH:.6f},{r_OH:.6f}\n')
        
    
    # print current step and time to stdout
    now = datetime.now()
    print(f'Finished step {l}'.ljust(20), now.strftime('%H:%M:%S'))
    l += 1
