...
# example usecase:
# python extract_precatalytic.py -m 5-HETE -H 10 -d 4.0 -l 20.0 -n 1 -e 6628
...
parser.add_argument(
    ...
    default=None,
    required=False,
    help='Specific frame to extract. Int.',
)
parser.add_argument(
    '-v', '--verbose', 
    type=bool,
...
# check molecule input is among the possible ligands for this system
lig_ID = args.molecule
lig_ID_list = ['AA', '5-HpETE', '5-HETE']
if lig_ID not in lig_ID_list:
    raise ValueError(f'Ligand {lig_ID} is not among the supported ligands: {lig_ID_list}')
...
# constants
# 
md_dir = os.path.abspath('../md_trajectory/')
frame_extraction_path = os.path.abspath('../preprocessing/frame_extraction/')
if not os.path.exists(frame_extraction_path):
    os.makedirs(frame_extraction_path)
traj_dir = os.path.join(md_dir, 'prod', 'out')
print(f'MD parent directory:\t{md_dir}')
print(f'MD trajectory location:\t{traj_dir}')
...
# locate .prmtop topology file
top_list = [os.path.join(md_dir, i) for i in os.listdir(md_dir) if i.endswith('.prmtop')]
if len(top_list) > 1:
    raise ValueError(f'More than one topology files was found in {md_dir}!')
print(f'loading prmtop\t\t{top_list[0]}')

# locate .nc trajectory files
prod_files = [x for x in os.listdir(traj_dir) if '.nc' in x]
traj_list = [os.path.join(traj_dir, i) for i in prod_files]
...
emda = EMDA()
emda.load_variant(
    parameters=top_list[0],
    trajectory=traj_list,
    variant_name=protein_ID
)
u = Universe(os.path.join(md_dir, top_list[0]), traj_list)

# total simulated time (each .nc is 10 ns long)
print(f'total time simulated: {10 * len(traj_list)} ns')
...
#
precat_pkl = os.path.join(md_dir, f'{protein_ID}_{lig_ID}_H{selection}_precat_analysis.pkl')
try:
    ...
    print(f'analyzing precatalytic structures for the trajectory in {md_dir}')
    emda.run()
...
# final results
if verbose:
    print(f'Frames that satisfy the dihedral criterion:\t{(len(criterion_1_idx)*100/(1000 * len(traj_list))):.2f}\t%')
    print(f'Frames that satisfy the distance criterion:\t{(len(criterion_2_idx)*100/(1000 * len(traj_list))):.2f}\t%')
    print(f'Frames that satisfy both criteria:\t\t{(len(criteria_idx)*100/(1000 * len(traj_list))):.2f}\t%')
...
# Note: precat_idx is 0-based indexing, not 1-based!
...
# if extra_frames are provided, add them to the extraction list (ad hoc)
if extra_frame is not None:
    random_precat_idx.append(extra_frame)

print(random_precat_idx)

# extract the corresponding selected frames
frame_path = os.path.join(frame_extraction_path, f'frames_H{selection}')
os.makedirs(frame_path)
trajectory_frame_extractor(u, random_precat_idx, folder=frame_path, outformat=".pdb", solvent=True)


# copy preprocessing_scripts folder to preprocessing
shutil.copytree('./preprocessing_scripts', '../preprocessing/preprocessing_scripts')
