...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        '--extra_residues', 
        nargs='+', 
        help='Residue numbers to include in the ML region. Example: \"662 663 664\"'
    )
    return p.parse_args()
...
def select_residues(complex_struct, substrate_res_list: List[PDB.Residue.Residue],
                    r_ml: float, r_het: float, include_h2o: bool, extra_residues_list: list) -> Set[Tuple]:
    ...
    def add_extra_residues(extra_res_list):
        '''
        Custom function. Adds extra residues from --extra_residues flag
        '''


        # obtain residues from pdb
        imposed_res_list = []

        # for every ligand (only 1, skip the first loop)
        for lig in substrate_res_list:
            model, chain, resinfo = lig.get_full_id()[1:4]
            seq = resinfo[1]
            chain_ent = complex_struct[model][chain]

            # obtain the id of each residue in the complex
            # and store the id of the -e extra_residues 
            for res in chain_ent:
                #print(res_id:= res.id)
                #res_id= res.id
                for extra_res in extra_res_list:
                    if res.id[1] == int(extra_res):
                        imposed_res_list.append(res.id) 

        # add specific residues (res_number)
        # for ALOX15 active QM region: 662, 663, 664, 544, 540, 365, 360
        #imposed_res_list = [
        #    ('H_OH1', 664, ' '),
        #    ('H_FE1', 663, ' '),
        #    ('H_IE1', 662, ' '), 
        ...
        for lig in substrate_res_list:
            ...
            chain_ent = complex_struct[model][chain]
            print('\n')
            print('Adding ad hoc residues:')
            for imposed_res in imposed_res_list:
                new_res = chain_ent[imposed_res]
                new_fid = new_res.get_full_id()
                selected_ids.add(new_fid)
                within_cutoff.add(new_fid)
                print(new_fid)
            print('\n')


    add_extra_residues(extra_residues_list)
...
def def_ml_region():
    ...
    selected_ids = select_residues(complex_struct, substrate_residues,
                                   ...
                                   args.include_H2O, args.extra_residues)
    # count raw atoms
